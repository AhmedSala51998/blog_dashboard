...
<body>
    ...
    $(document).on('click', '.add-section-btn', function() {
        ...
        let sectionId = sectionCount[articleId];

        let sectionHtml = `
        <div class="section-form" id="section-${articleId}-${sectionId}">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <h6>جزء ${sectionId}</h6>
                <button type="button" class="btn btn-sm btn-outline-danger remove-section" data-article="${articleId}" data-section="${sectionId}">
                    <i class="fas fa-times"></i> إزالة
                </button>
            </div>
            <div class="mb-3">
                <label class="form-label">عنوان الجزء</label>
                <input type="text" class="form-control" name="articles[${articleId}][sections][${sectionId}][title]">
            </div>
            <div class="mb-3">
                <label class="form-label">محتوى الجزء</label>
                <textarea class="form-control" name="articles[${articleId}][sections][${sectionId}][content]" rows="3"></textarea>
            </div>
        </div>
        `;
