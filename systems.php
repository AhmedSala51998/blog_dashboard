...
// دالة لمعالجة ملفات PDF واستخراج البيانات
function processPdfFile($file_path, $system_id) {
    global $conn;
    
    // التحقق من وجود مكتبة معالجة PDF
    if (!class_exists('\Smalot\PdfParser\Parser')) {
        // إضافة مكتبة PDF Parser إذا لم تكن موجودة
    ...
    try {
        ...
        // هنا نستخدم نمطًا بسيطًا للكشف عن المواد (مادة 1، مادة 2، إلخ)
        $pattern = '/\b(مادة|المادة)\s+(\d+)\b/i';
        ...
        // إذا تم العثور على مواد
        if (!empty($matches[0])) {
            // الحصول على جميع المواضع
            $positions = [];
            foreach ($matches[0] as $match) {
                $positions[] = $match[1];
            }
            
            // إضافة نهاية النص كآخر موقع
            $positions[] = strlen($text);
            ...
            for ($i = 0; $i < count($positions) - 1; $i++) {
                ...
                // استخراج نص المادة
                $article_text = trim(substr($text, $start_pos, $end_pos - $start_pos));
                
                // الحصول على رقم المادة
                $article_number = $matches[2][$i][0];
                
                // فصل العنوان عن المحتوى (أول سطر هو العنوان)
                $lines = explode("\n", $article_text, 2);
                $title = trim($lines[0]);
                $content = isset($lines[1]) ? trim($lines[1]) : "";
                
                // البحث عن الأجزاء داخل المادة
                $section_pattern = '/\b(جزء|الجزء)\s+(\d+)\b/i';
                preg_match_all($section_pattern, $content, $section_matches, PREG_OFFSET_CAPTURE);
                
                $sections = [];
                
                // إذا تم العثور على أجزاء
                if (!empty($section_matches[0])) {
                    // الحصول على جميع مواضع الأجزاء
                    $section_positions = [];
                    foreach ($section_matches[0] as $match) {
                        $section_positions[] = $match[1] - $start_pos;
                    }
                    
                    // إضافة نهاية المحتوى كآخر موقع
                    $section_positions[] = strlen($content);
                    
                    // معالجة كل جزء
                    for ($j = 0; $j < count($section_positions) - 1; $j++) {
                        $section_start = $section_positions[$j];
                        $section_end = $section_positions[$j + 1];
                        
                        // استخراج نص الجزء
                        $section_text = trim(substr($content, $section_start, $section_end - $section_start));
                        
                        // الحصول على رقم الجزء
                        $section_number = $section_matches[2][$j][0];
                        
                        // فصل العنوان عن المحتوى
                        $section_lines = explode("\n", $section_text, 2);
                        $section_title = trim($section_lines[0]);
                        $section_content = isset($section_lines[1]) ? trim($section_lines[1]) : "";
                        
                        // البحث عن الأجزاء الفرعية
                        $subsection_pattern = '/\b(الفرع|فرع|البند|بند)\s+(\d+)\b/i';
                        preg_match_all($subsection_pattern, $section_content, $subsection_matches, PREG_OFFSET_CAPTURE);
                        
                        $subsections = [];
                        
                        // إذا تم العثور على أجزاء فرعية
                        if (!empty($subsection_matches[0])) {
                            // الحصول على جميع مواضع الأجزاء الفرعية
                            $subsection_positions = [];
                            foreach ($subsection_matches[0] as $match) {
                                $subsection_positions[] = $match[1] - $section_start;
                            }
                            
                            // إضافة نهاية المحتوى كآخر موقع
                            $subsection_positions[] = strlen($section_content);
                            
                            // معالجة كل جزء فرعي
                            for ($k = 0; $k < count($subsection_positions) - 1; $k++) {
                                $subsection_start = $subsection_positions[$k];
                                $subsection_end = $subsection_positions[$k + 1];
                                
                                // استخراج نص الجزء الفرعي
                                $subsection_text = trim(substr($section_content, $subsection_start, $subsection_end - $subsection_start));
                                
                                // الحصول على رقم الجزء الفرعي
                                $subsection_number = $subsection_matches[2][$k][0];
                                
                                // فصل العنوان عن المحتوى
                                $subsection_lines = explode("\n", $subsection_text, 2);
                                $subsection_title = trim($subsection_lines[0]);
                                $subsection_content = isset($subsection_lines[1]) ? trim($subsection_lines[1]) : "";
                                
                                // إضافة الجزء الفرعي إلى المصفوفة
                                $subsections[] = [
            ...
            // إذا لم يتم العثور على مواد، قم بمعالجة النص كنظام واحد
            $lines = explode("\n", $text, 2);
            $title = "مستند PDF مستورد";
            $content = $text;
        ...
        // إضافة المواد والأجزاء إلى قاعدة البيانات
    ...
    if (isset($_POST['upload_pdf']) && isset($_FILES['pdf_file'])) {
        ...
        if ($_FILES['pdf_file']['error'] == UPLOAD_ERR_OK) {
            ...
            if ($file_type == 'application/pdf') {
                ...
                $file_name = 'pdf_' . time() . '_' . uniqid() . '.pdf';
                $file_path = 'uploads/' . $file_name;
                
                // التحقق من وجود مجلد المرفقات
                ...
                if (move_uploaded_file($_FILES['pdf_file']['tmp_name'], $file_path)) {
                    // معالجة ملف PDF واستخراج البيانات
                    if (processPdfFile($file_path, $system_id)) {
                        $_SESSION['message'] = "تم رفع ملف PDF واستخراج البيانات بنجاح!";
                        $_SESSION['message_type'] = "success";
                    } else {
                        $_SESSION['message'] = "حدث خطأ أثناء معالجة ملف PDF. يرجى التحقق من تنسيق الملف.";
                ...
                $_SESSION['message'] = "نوع الملف غير مدعوم. يرجى رفع ملف PDF فقط.";
                $_SESSION['message_type'] = "danger";
            }
        } else {
            $_SESSION['message'] = "حدث خطأ أثناء رفع الملف: " . $_FILES['pdf_file']['error'];
            $_SESSION['message_type'] = "danger";
        }
    }
                ...
                <div class="content">
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            <h5 class="mb-0"><i class="fas fa-file-pdf"></i> استيراد بيانات من ملف PDF</h5>
                        </div>
                        <div class="card-body">
                            <p class="card-text">يمكنك رفع ملف PDF يحتوي على بيانات النظام والمواد والأجزاء ليتم استيرادها تلقائياً.</p>
                            
                            <form method="post" enctype="multipart/form-data">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <label for="system_select" class="form-label">اختر النظام</label>
                                        <select class="form-select" id="system_select" name="system_id" required>
                                            <option value="">-- اختر نظام --</option>
                                            <?php
                                            // استعادة قائمة الأنظمة
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input class="form-control" type="file" id="pdf_file" name="pdf_file" accept=".pdf,application/pdf" required>
                                        <div class="form-text">الملف المسموح به: PDF فقط، الحد الأقصى: 10 ميجابايت</div>
                                    </div>
                                </div>
                                <div class="mt-3">
                                    <button type="submit" name="upload_pdf" class="btn btn-primary">
                                        <i class="fas fa-upload"></i> رفع واستيراد البيانات
                                    </button>
                                </div>
                            </form>
                        </div>
